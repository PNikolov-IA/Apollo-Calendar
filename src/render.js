...
const addWeeklySlots = (table) => {
    let year = generate.currentMonthAndYear[1];
    let month = time.months[generate.currentMonthAndYear[0]];
    for (let hours = 9; hours <= 18; hours++) {
        let hr = hours.toString().padStart(2,'0');
        let calendarSlot = $(`<tr hours="${hr}"></tr>`);
        let classOnSlot;
        let weekElements = $(`<td class="dailySlot">${hr}:00-${hours+1}:00</td>`);
        calendarSlot.append(weekElements);
        for (let d = 0; d < 7; d++) {
            let day = generate.currentWeekDays[d];
            const events = db.findSlot(year, month, day, hr);
            !events[0] ? classOnSlot = 'daily-event': classOnSlot = 'event-item';
            let dayElement = $(`<td data-year="${year}" data-month="${month}" data-day="${day}" data-hour="${hr}">${events[0] || ''}</td>`).addClass(classOnSlot);
    ...
    renderMonthlyView: (currentMonth, currentYear) => {
        ...
        for (let weeks = 1; weeks < 6; weeks++) {
            ...
            for (let days = 1; days <= 7; days++) {
                let eventCount = db.findEventsCount(currentYear, time.months[currentMonth], counterDays+1);
                let displayCount = eventCount ? `<i class="fa fa-calendar" aria-hidden="true"></i> x ${eventCount}` : '';
